...
if __name__ == "__main__":
    ...
    if export_data:
        ...
        masked_output = masked_output * set_dtype(seqlen_mask.unsqueeze(-1).cuda(), dtype)
        print("max diff k:", torch.max(torch.abs(output_k - k)).cpu())
        print("diff num k:", torch.sum(torch.abs((output_k - k) / output_k) > 1e-2).cpu())
        print("max diff v:", torch.max(torch.abs(output_v - v)).cpu())
        print("diff num v:", torch.sum(torch.abs((output_v - v) / output_v) > 1e-2).cpu())
