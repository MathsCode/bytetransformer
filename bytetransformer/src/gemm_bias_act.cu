...
template <>
__global__ void add_bias_relu<float>(float *output, const float *bias, const int M, const int N) {
  int row_offset = blockIdx.x * N;
  for (int tid = threadIdx.x; tid < N; tid += blockDim.x) {
    float out = output[row_offset + tid] + __ldg(&bias[tid]);
    output[row_offset + tid] = relu(out);
  }
}
...
template <>
__global__ void add_bias_relu<__half>(__half *output, const __half *bias, const int M,
                                      const int N) {
  if (N % 4 != 0) {
    half2 *output_ptr = (half2 *)output;
    const half2 *bias_ptr = (const half2 *)bias;
    ...
    for (int tid = threadIdx.x; tid < N / 2; tid += blockDim.x) {
      half2 out = __hadd2(output_ptr[row_offset + tid], __ldg(&bias_ptr[tid]));
      output_ptr[row_offset + tid] = relu(out);
    }
  } else {
    float2 *output_ptr = (float2 *)output;
    const float2 *bias_ptr = (const float2 *)(bias);
    ...
    for (int tid = threadIdx.x; tid < N / 4; tid += blockDim.x) {
      ...
      val.h[0] = relu(__hadd2(val.h[0], bias_val.h[0]));
      val.h[1] = relu(__hadd2(val.h[1], bias_val.h[1]));
...
// void cublas_gemm_bias_relu(const __half *A, const __half *B, __half *C, const __half *bias, int m,
//   int k, int n, cudaStream_t stream, cublasHandle_t cublas_handle,
//   int cublasAlgo) {
// dense_layer_kernel_launcher(A, B, C, m, k, n, cublas_handle, stream, cublasAlgo);
// add_bias_relu<<<m, n / 8, 0, stream>>>(C, bias, m, n);
// }
void cublas_gemm_bias_relu(const __half *A, const __half *B, __half *C, const __half *bias, int m,
                           int k, int n, cudaStream_t stream, cublasHandle_t cublas_handle,
                           int cublasAlgo) {
  ...
  add_bias_relu<<<m, block, 0, stream>>>(C, bias, m, n);
}
...
void gemm_bias_relu<__half>(const half *A_, const half *B_, half *C_, const half *bias_, int m_,
                            ...
                            int cublasAlgo, int arch) {
  if (m_ < 8)
    cublas_gemm_bias_relu(A_, B_, C_, bias_, m_, k_, n_, stream, cublas_handle, cublasAlgo);
  else {
#if (__CUDACC_VER_MAJOR__ >= 11)
    const ElementComputeEpilogue alpha = ElementComputeEpilogue(1.0f);
    const ElementComputeEpilogue beta = ElementComputeEpilogue(1.0f);
...
#define _inst_k 4
      ...
      GEMM_TYPE(_block_m, _block_n, _block_k, _warp_m, _warp_n, _warp_k, _inst_m, _inst_n, _inst_k,
                NumStages)
      GEMM_BIAS_RELU(_block_m, _block_n, _block_k, _warp_m, _warp_n, _warp_k, _inst_m, _inst_n,
                     _inst_k)
      gemmBiasRelu_op;
      GEMM_INIT(_block_m, _block_n, _block_k, _warp_m, _warp_n, _warp_k, _inst_m, _inst_n, _inst_k)
#undef _inst_m
#undef _inst_n
#undef _inst_k
      CUTLASS_CHECK(gemmBiasRelu_op.initialize(args, cutlass_workspace_));
      CUTLASS_CHECK(gemmBiasRelu_op(stream));
    } else if (arch == 75 && (m_ > 192 && m_ <= 3456)) {
      using SmArch = cutlass::arch::Sm75;
#define _inst_m 16
#define _inst_n 8
